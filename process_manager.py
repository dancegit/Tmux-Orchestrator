...
class ProcessManager:
    ...
    def __init__(self, max_runtime: int = 10800, monitor_interval: int = 30):
        ...
        self.status_callback = None  # Optional callback for DB updates
        self.tmux_manager = None  # For zombie checks (will be set by scheduler)
    ...
    def set_status_callback(self, callback: Callable[[int, str, str], None]):
        """Set callback for status updates (backward-compatible; optional)."""
    ...
    def register(self, project_id: int, pid: int, cmd: Optional[str] = None, session_name: Optional[str] = None):
        ...
        Args:
            ...
            session_name: Optional tmux session name for zombie detection
        """
        with self._lock:
            self.active_processes[project_id] = {
                ...
                'session_name': session_name  # New field for zombie detection
            }
        logger.info(f"Registered process {pid} for project {project_id} (cmd: {cmd}, session: {session_name})")
    ...
    def kill(self, project_id: int, force: bool = False, reason: str = "Manual termination") -> bool:
        ...
        try:
            ...
            if self.status_callback:
                try:
                    self.status_callback(project_id, 'failed', reason)
                except Exception as e:
                    logger.error(f"Error calling status callback for project {project_id}: {e}")
    ...
    def _monitor(self):
        ...
        while not self._stop:
            ...
            for project_id, info in process_items:
                ...
                # New: Zombie detection (PID alive but tmux session dead)
                if self.tmux_manager and info.get('session_name'):
                    session_name = info['session_name']
                    if not self.tmux_manager.session_exists(session_name):
                        logger.warning(f"Zombie detected for project {project_id}: PID {pid} alive but session {session_name} dead")
