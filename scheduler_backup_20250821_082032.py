...
class TmuxOrchestratorScheduler:
    ...
    def detect_and_reset_phantom_projects(self) -> int:
        ...
        This prevents queue blockage from failed auto_orchestrate.py launches.
        
        Returns: Number of phantom projects reset
        """
        cursor = self.conn.cursor()
        cursor.execute("""
            SELECT id, started_at, orchestrator_session, main_session, spec_path
            FROM project_queue 
            WHERE status = 'processing'
        """, ())
        ...
        for project_id, started_at, orch_session, main_session, spec_path in processing_projects:
            is_phantom = False
            phantom_reason = ""
            
            # Check 1: No session name set (auto_orchestrate.py failed before session creation)
            if not orch_session and not main_session:
                # Give it some time for session creation (2 minutes max)
                if started_at and time.time() - float(started_at) > 120:  # 2 minutes
                    is_phantom = True
                    phantom_reason = "No session created after 2 minutes"
            
            # Check 2: Session name set but tmux session doesn't exist
            elif orch_session and not self._session_exists(orch_session):
                is_phantom = True
                phantom_reason = f"Session '{orch_session}' no longer exists"
            elif main_session and not self._session_exists(main_session):
                is_phantom = True
                phantom_reason = f"Main session '{main_session}' no longer exists"
            
            # Check 3: Project running for more than 10 minutes without session
            elif not orch_session and not main_session and started_at:
                if time.time() - float(started_at) > 600:  # 10 minutes
                    is_phantom = True
                    phantom_reason = "No session after 10 minutes"
            
            if is_phantom:
                logger.warning(f"PHANTOM PROJECT DETECTED: Project {project_id} - {phantom_reason}")
                self.mark_project_complete(
                    project_id, 
                    success=False, 
                    error_message=f'Phantom project reset: {phantom_reason}'
                )
                reset_count += 1
                logger.info(f"Reset phantom project {project_id}")
            ...
            try:
                ...
                if next_project:
                    ...
                    try:
                        ...
                        # ENHANCED: Add timeout and comprehensive logging
                        result = subprocess.run(cmd, check=True, capture_output=True, text=True, timeout=3600)
                        
                        # Log execution details for debugging
                        logger.debug(f"auto_orchestrate.py execution completed for project {next_project['id']}")
